...
class CompileBackendMIGRAPHX(compile_backend.CompileBackend):
    ...
    def compile(self, config, dataloader=None):
        ...
        input_tensors = []
        # Convert tf/pytorch model to onnx model
        ...
        if self.framework == "Tensorflow":
            ...
            # Ensure model_path is a directory, not a file path
            model_dir = os.path.dirname(model_path)  # Extract directory path
            model_name = os.path.basename(model_path).replace('.pt', '')  # Extract the model name without .pt
    
            # Construct the ONNX path
            model_onnx_path = os.path.join(model_dir, self.model_name + '.onnx')
            if not os.path.exists(model_onnx_path):
                torch_to_onnx.torch_to_onnx(model_path, model_onnx_path)
        batch_sizes = config['workload']['batch_sizes']
        updated_model_paths = []
        for batch_size in batch_sizes:
            model_path_for_batch_size = model_onnx_path.rsplit("/", 1)
            batch_size_with_precision = str(batch_size) + '-' + model_precision.lower()
            model_onnx_path_set_batch_size = os.path.join(model_path_for_batch_size[0], batch_size_with_precision, model_path_for_batch_size[1])
            model_dir = os.path.join(model_path_for_batch_size[0], batch_size_with_precision)
            ...
            if not os.path.exists(model_path):
                ...
                if model_precision == "FP16":
                    print(f"*****************Quantizing model to FP16*****************")
